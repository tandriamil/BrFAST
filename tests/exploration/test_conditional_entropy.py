...
from brfast.exploration.conditional_entropy import (
    get_best_conditional_entropic_attribute,
    best_conditional_entropic_attribute, ConditionalEntropy)
...
class TestGetBestConditionalEntropicAttribute(unittest.TestCase):
    ...
    def test_get_best_entropic_attribute(self):
        ...
        first_best = get_best_conditional_entropic_attribute(
        ...
        second_best = get_best_conditional_entropic_attribute(
            self._dataset, current_attributes=AttributeSet({ATTRIBUTES[1]}),
            candidate_attributes=self._attribute_set)
        self.assertEqual(second_best, ATTRIBUTES[0])

        third_best = get_best_conditional_entropic_attribute(
            self._dataset, current_attributes=AttributeSet({ATTRIBUTES[1],
                                                            ATTRIBUTES[0]}),
            candidate_attributes=self._attribute_set)
        self.assertEqual(third_best, ATTRIBUTES[2])

        no_more_available = get_best_conditional_entropic_attribute(
            self._dataset, current_attributes=AttributeSet({
    ...
    def test_get_best_entropic_attribute_every_attribute_already_taken(self):
        result = get_best_conditional_entropic_attribute(
    ...
    def test_get_best_entropic_attribute_non_existent_attribute(self):
...
class TestBestConditionalEntropic(unittest.TestCase):
    ...
    def test_best_conditional_entropic_attribute(self):
        ...
        # unique values
        best_cond_ent_attr = best_conditional_entropic_attribute(
    ...
    def test_best_conditional_entropic_attribute_non_existent_attribute(self):
        self._attribute_set.add(NON_EXISTENT_ATTRIBUTE)
        with self.assertRaises(KeyError):
            best_conditional_entropic_attribute(
                self._df_w_one_fp_per_browser,
